...
**T-01 已關門，下一步是文案大改（使用者拍板的三步順序）。** ① 126 條測試解耦 — 完成；② 大改 `data/beats/*.json` 的 `text` 與 `endings.json` 的頁面；③ 補 `locations.json` 的 48 個空 `desc`（順手收掉 K-238）。**改文案前先讀「T-01 結案總結與文案大改的安全區」那一節**——哪些改了不會撞測試、哪些標籤是契約不是文案，都在那裡。

**文案第一版（`db9bf68`）與第一次收斂（`fe72d7f`）都已驗收通過，機器層全綠。** `fe72d7f` 只收了 12 個檔裡的 3 個（`endings.json`／`indulgence_exits.json`／`ch3_d39_d45.json`），其餘九個仍是第一版文風，套語密度分裂——三個夜間檔最高（`ch2_nights` 9.1、`ch1_nights` 6.0 每千字），而那是全作最要命的恐怖體驗場。**下一輪照「T-02 文案套語清理工單」做**，含密度表、套語清單、不能踩的線與驗收方式。
...
## 現況：方向對，但 12 個檔只改了 3 個

`fe72d7f` 改了 `endings.json`、`indulgence_exits.json`、`ch3_d39_d45.json` 三個檔，手法正確——刪贅詞、換具體動詞、對白回到人話：

- 「許先生掐滅了手中的菸頭，眼神深邃地看著你……用極其沙啞的聲音低語」→「許先生按熄了菸頭，看了你一眼……只是低聲說」
- 「黑暗的深淵漠然吞噬了一切，並未對你的回答作出任何回應」→「深淵沒有任何回應」
- 「那張面孔赫然正是你自己」→「那張臉就是你自己」

**沒有矯枉過正**：`endings.json` 句長平均 26.4 → 17.2 字、標準差 11.5 → 7.5，**變異係數維持 0.44 不變**——整體變短但節奏起伏還在，不是削成平板。這點量過，不必再擔心。

問題是剩下九個檔還是第一版的文風，**密度現在是分裂的**。

## 一、套語密度表（每千字命中數）

掃描口徑見本節末。`text` 欄位，含 `on_enter`／`on_place`／結局頁。

| 檔 | 字數 | 密度 | 狀態 |
|---|---|---|---|
| `endings.json` | 1,948 | **0.0** | ✅ 已改 |
| `indulgence_exits.json` | 483 | **0.0** | ✅ 已改 |
| `ch3_d39_d45.json` | 4,396 | 1.4 | ✅ 已改 |
| `ch2_d16_d22.json` | 4,369 | 1.8 | 未改 |
| `ch1_d01_d03.json` | 2,119 | 1.9 | 未改 |
| `ch2_d27_d32.json` | 2,688 | 2.6 | 未改 |
| `ch1_d04_d15.json` | 6,673 | 2.7 | 未改 |
| `ch2_d23_d26.json` | 3,130 | 2.9 | 未改 |
| `ch3_d33_d38.json` | 2,783 | 2.9 | 未改 |
| `ch3_nights.json` | 637 | 3.1 | 未改 |
| `ch1_nights.json` | 2,322 | **6.0** | 未改 |
| `ch2_nights.json` | 1,433 | **9.1** | 未改 |

**最華麗的是三個夜間檔**——那是全作最要命的恐怖體驗場。結局已經素到見骨，夜裡還在「轟然」「赫然」，一路玩下來會感覺到接縫。

## 二、套語清單（全 `data/beats/*.json` 累計）

| 詞 | 次數 | 樣本 |
|---|---|---|
| `彷彿` | 23 | `ch1_d01_d03 > photos`：「**彷彿**整座小鎮的那一代孩子在一夜之間都被抹去了」 |
| `赫然` | 23 | `ch1_d04_d15 > read`：「你在泛黃的紙頁上**赫然**找到了自己的名字」 |
| `悄然` | 10 | `ch1_d01_d03 > n3_map_opens`：「三個隱沒在深巷與林道中的幽暗標記**悄然**浮現」 |
| `緩緩` | 7 | `ch1_nights > n_queue_ch1`：「前方的身影**緩緩**轉過頭來，面無表情地低語」 |
| `猛然` | 6 | `ch1_d04_d15 > ask`：「她**似乎猛然意識到**失言，立刻抿緊嘴唇」（雙重濾鏡詞，直接寫她抿嘴就夠） |
| `驟然` | 4 | — |
| `難以言喻` | 3 | — |
| `靜靜地`／`宛如`／`深深`／`意識到`／`莫名` | 各 2 | — |

**判準不是「這些詞不能用」，是它們成群出現時整段就變成機器味。** 單獨一個「彷彿」沒事，一頁三個就有事。

## 三、已改檔裡的殘留（同檔內具體度落差）

`indulgence_exits.json` 四條縱慾出口，兩條有實體、兩條停在抽象：

| 條 | 現況 | 判定 |
|---|---|---|
| `x_smash` | 「碎瓷片濺了一地」 | ✅ 具體 |
| `x_binge` | 「冷飯剩菜」「胃部脹得發痛」 | ✅ 具體 |
| `x_violence` | 「對**身邊的人**動了粗」 | ❌ 沒有對象、沒有動作 |
| `x_lust_ajie` | 「衝動和恐懼**交織**在一起」「理智被撕開了一道口子」 | ❌ 抽象配對＋比喻 |
| `x_lust_awei` | 「眼神裡滿是**困惑與無措**」 | ❌ 配對抽象名詞 |

縱慾出口是玩家會反覆撞見的段落，四條裡兩條具體兩條抽象，落差看得出來。

## 四、建議順序

1. **`ch1_nights` / `ch2_nights` / `ch3_nights`**——密度最高、戲份最重，先修這三個。
2. **`ch1_d04_d15`**——6,673 字、131 段，全作最大宗。
3. 其餘白天檔照密度由高到低。
4. 順手收 `indulgence_exits` 那三條的具體度落差。

## 五、改的時候不能踩的線

- **`endings.json > pages[].narrative_beats` 是契約不是文案。** 改寫結局頁時該頁承諾的敘事要素要留著；要素真的拿掉了就同步改標籤，別讓標籤說謊。封閉字彙住 `data_loader.gd > LEGAL_NARRATIVE_BEAT_TAGS`，Lint 21 擋。
- **`reject_reason_tag` 同理**，四個標籤住 `LEGAL_REJECT_REASON_TAGS`，Lint 12 擋。
- **刻意保留的字面不要動**：`test_p3a.gd:177` 的 `"血還是新的"`（反向斷言，就是要禁止舊名復活）、`card_types.json` 與 `relation_scale.json` 的封閉語彙。
- **`label` 與 `title` 不要跟著變長**。`db9bf68` 那一版沒動 `label`（250 條統計前後完全一致），`title` 最長還從 19 降到 13，chrome 因此沒有版面風險。維持這個做法。
- **不要碰結構**。`db9bf68` 把 `d28_pm_uncle` 的開發者 `note` 從 `slots[1]` 搬到 beat 層，純文案 commit 不該動結構（記在 K-240）。

## 六、驗收方式

1. 重跑密度掃描，**目標是各檔都降到 1.5 以下**，且沒有任何檔仍在 3.0 以上。
2. 全套 headless 33 套 exit 0。
3. UI sim `-Background`，附 run id。
4. 抽讀三段確認沒有削成平板：量句長變異係數，改動前後應維持在 0.4 上下（不是越小越好）。
5. 長度別再往上長——`db9bf68` 已經把 `text` 從 18k 拉到 37k，K-240 的版面風險還沒做人工驗收。

### 掃描口徑（可重現）

遞迴走訪 `data/beats/*.json` 與 `data/endings.json`，只取 `text` 欄位，統計下列六類命中數 ÷ 總字數 × 1000：

- **膨脹形容詞**：細膩／精緻／深刻／雋永／震撼／極致／絕美／唯美／璀璨／耀眼／動人／驚豔／扣人心弦／刻骨銘心／淋漓盡致／恰到好處／無可取代／分毫不差／無邊無際／無聲無息
- **小說套語**：空氣彷彿凝結／時間彷彿靜止／心頭一緊／喉頭一哽／鼻頭一酸／眼眶泛紅／嘴角勾起／幾不可聞／緩緩開口／輕聲說道／深吸一口氣／瞳孔微縮／指尖顫抖／目光交會／意味深長／若有似無／不易察覺
- **標誌性副詞與片語**：說不出的／道不盡的／難以言喻／莫名的／空氣中瀰漫／內心深處／宛如／彷彿／猛然／悄然／驟然／轟然／溘然／黯然／毅然／決然／赫然／儼然
- **宏大名詞**：浩瀚／璀璨／瑰寶／殿堂／盛宴／樂章／史詩／篇章／里程碑／縮影／光輝／靈魂深處／印記
- **濾鏡詞**：感覺到／意識到／注意到／不禁
- **四字成語堆疊**：白駒過隙／蒸蒸日上／有聲有色／風雨飄搖／撒手人寰／長相廝守／結為連理／結髮為伴／平淡相守／苦心經營／舉手投足／溘然長逝／急劇惡化／來勢洶洶／彈指而過／悄然而逝

---

# T-01 結案總結與文案大改的安全區（verifier，2026-08-30）

## 這件事是為了什麼

T-01 的目的不是把測試改漂亮，是**把「改文案」與「弄紅測試」脫鉤**，讓接下來的敘事大改變成純寫作。這一節記的是「現在可以放心改什麼」，給下一個動文案的人看。
...
| 分類 | 數量 | 處置 |
|---|---|---|
| A 類（呈現契約） | 59 | 改讀資料 ＋ `assert(not ...is_empty())` 防空跑守衛 |
| B 類（敘事契約） | 30 | `endings.json` 的 `narrative_beats` 語意標籤 ＋ Lint 21 ＋ `SCHEMA.md` 登記為非翻譯欄位 |
| 偽陽性（斷言訊息／合成 fixture／日誌） | 37 | 原樣保留 |

耦合重掃 **126 → 10**，第二輪再降到 9，剩下全是錯誤訊息格式字串與合成探針，verifier 判定合理。四輪 review 的 D1～D5／E1～E3／F1～F4 全數結案，新增 K-237～K-239 皆不擋關門。

## 2 可以放心改的（有實測證據）

| 要改的東西 | 證據 | 結論 |
|---|---|---|
| `data/beats/*.json` 與 `endings.json` 的 `text`（580 條） | MT-1：全部換成無關新句 → **33 套 headless 全綠** | **隨便改**，不會撞測試 |
| `cards.json`／`locations.json` 的 `name`、beats 的 `title`／`slots[].label`（632 條） | MT-1b：全部改名 → 32 綠 1 紅 | **可以改**，唯一的紅是設計規則不是耦合，見下 |

**那唯一一條紅**：`test_p3a` 的「對位卡名稱必須包含對應白天地點名」。這是真實資料不變量（夜間對位卡 `k_night_*` 的 `name` 要含白天地點的 `name`），改名時兩側要一起改，不是解耦沒做完。

## 3 改文案時仍然會咬人的地方

- **B 類語意標籤**：`endings.json` 的 `pages[].narrative_beats` 是**契約不是文案**。改寫結局頁的 `text` 時，該頁原本承諾的敘事要素要留著；要素真的拿掉了，就同步改 `narrative_beats`，別讓標籤說謊。封閉字彙 19 個住 `data_loader.gd > LEGAL_NARRATIVE_BEAT_TAGS`，Lint 21 會擋。
- **`reject_reason_tag`**：同理，四個標籤住 `LEGAL_REJECT_REASON_TAGS`，Lint 12 會擋。改夜間地點的拒絕理由時標籤要跟著語意走。
- **刻意保留的字面**：`test_p3a.gd:177` 的 `"血還是新的"`（反向斷言，就是要禁止這個舊名復活）、`card_types.json` 與 `relation_scale.json` 的封閉語彙。看到不要「順手解耦」。

## 4 下一步的順序（使用者已拍板）

1. ~~126 條測試解耦~~ — **完成**（本節）。
2. **大改 `data/beats/*.json` 的 `text` 與 `endings.json` 的頁面**——「更故事性」的主戰場，第 2 節已證明是安全區。
3. **補 `locations.json` 的 `desc`**——48 個地點的 `desc` 目前全部是空的，補上等於白賺 48 段氛圍文字。`SCHEMA.md` 已把 `desc` 登記為可翻譯欄位，寫進去就會被 i18n 抽取工具帶走。

**做第 3 步時順手把 K-238 一起收掉**：`SCHEMA.md > 可翻譯欄位清單` 的 `locations.json` 那一列漏了 `reject_reason`，反正都要動同一列。K-237（tag 與 `requires` 的對應無人強制）依原排程留在 i18n 管線階段。

## 5 一個未補齊的證據

verifier **沒有親自跑過改名後的 33 套**——批次寫入 `data/*.json` 被本機權限層擋掉兩次。改以「632 處逐檔筆數獨立列舉比對」（十三個檔全部對上）加上四組單點變異實測替代，判定證據充分。若之後要補這一層，改名腳本的邏輯是：遞迴走訪，物件同時有 `id` 與非空 `name` 時改 `name`；任何層級的 `label`／`title` 直接改。
