...
| P3-C 夜間內容流程 | 📐 | 唯一 `night_beats_at()` resolver、fixed／D3／D24、睡覺停拍與三種推進按鈕，收 K-30／33／69 |
| P3-D 夜間地點清單與詳情 UI | 📐 | 穩定文字清單、四種狀態文字＋teaser 鎖定案例、隱藏價碼、可讀 LOCKED 詳情、風險 warning；不做座標地圖 |
| P3-E 對位系統 | 📐 | 白天正確地點免費確認、共用 knowledge、多對一命名、matched 狀態純衍生 |
| P3-F 全流程驗收 | 📐 | 28-row matrix、第一／二輪重演、P1／P2 回歸、UI 契約與人工體感驗收 |
...
- 2026-08-20：**P3 三份文件的規格審核完成，九條缺口落檔為 K-73～K-81。** 規格與資料對得上（28 row／12 對位／10 卡／16 夜間限定／6 門檻／14 收費 row 全部實測相符），**問題全部在沒有人把數值後果算過**：**K-73**——現行資料的發狂卡只有 marker cost 與 `n3_map_opens` 三個槽兩個來源，後者 P3-C 要刪、前者 P3-B 改成終身首次，因此**第一輪走滿 14 個收費 row 的玩家，第二輪拿不到新卡**（漏掉的 row 第二輪首次去仍會發；但 A／B／C 三種模擬都走滿，所以對現行基準而言就是 0）。**第二輪的故事還沒寫**，新內容可能自己帶來源——要防的是在那之前先把「第二輪 0 張」當成正確行為釘死；**K-74**——`n_plaza` 是 14 個收費 row 裡唯一掛 night-layer fixed 的，D15 讓它永久免費，**第一輪供給 14 → 13**，而 `實作規格書.md > P3-F` 寫「仍逐項對上 P2 模擬」，與方針／測試指南的「不一致就更新基準」互相打架。**K-73／K-74 要在 P3-A 開工前處理**，其餘七條各自綁在對應子階段開工時；清單見 `驗證後已知問題.md > 一、待修清單`。
...
- **下一步實作 P3-A 夜間資料真值化——但先過 K-73／K-74 那兩條文件閘。** P3-C 才處理 K-69（夜間睡覺文字沒上畫面）並解除 K-68 的封鎖；不得為了先關 K-69 跳過 P3-A／P3-B 的資料與狀態前置。
