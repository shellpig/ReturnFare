...
| P4-C 委託 UI 與首個案例 | 🟦 | 委託候選／預覽／確認、第一張人物卡教學、D17～19 處方案例均已實作；K-124 結案、25 套 headless 與 5 個新 UI variants 全綠。完整 UI sim 因 K-125 modal 輸入問題為 98／102 variants、76／79 contracts，待修後關門 |
...
- 2026-08-27：**P4-C 已實作，K-124 經 verifier 複驗結案；新發現 K-125 阻塞完整 UI 關門。** `4b0ba1c` 以 fixture-only madness 正規化修復 8 組精確張數情境，補張數／clock／day／phase 語意守衛，零人物卡改按 type 計數，並同批結 K-112（baseline 只在明示 `--p3a-baseline` 時產生）。verifier 自跑 fixture 生成 exit 0、baseline mtime 不變、25 套 headless 全綠；完整 UI sim 跑到 102 variants／79 contracts，98 variants 通過，4 個失敗同源於 D17 首張人物卡彈出的教學 modal 遮住底層 `beat_advance`，`drain_beats()` 卻只看 `is_visible_in_tree()` 而誤報無限迴圈。**下一步只修 K-125 的 UI sim modal 優先序，先重跑 4 案再跑全套；達 102／102、79／79、0 failed 後才把 P4-C 轉 ✅ 並進 P4-D。**
