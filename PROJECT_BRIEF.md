...
| P2-D 視野門檻與發瘋 BE | ✅ | 達 `madness_cap` 立即 BE、批次發卡只結束一次、共用既有 `end_run()` 收尾、BE 走獨立顯示分支（`test_p2d.gd` 全綠、15 套 headless 全綠、UI 全套 79／61 無迴歸）。⚠️ **驗收覆蓋有六條尾巴（K-63～K-68）**，其中 K-63／K-64 是「真資料那一半沒釘住」，排 P2-E 動工前修 |
...
- 2026-08-20：**P2-D 驗證完成，15 套 headless exit 0、UI 全套 79 variants／61 contracts／0 failed checks（`main.gd` 有動所以 UI 全套是必要的）。** 規則層對得上規格：cap 檢查掛在 `gain_card()` 之後、`open_night_marker()` 發完整批才檢查一次、走既有 `end_run()` 不開第二條路徑、BE 走獨立顯示分支、重置回第 1 天且 meta 保留。實作者另外自己抓到一個邊界並修掉——BE 在 `open_night_marker()` 中途觸發時，`main.gd` 原本還會繼續開地點面板（`if _is_showing_ending: return`）。**Review 抓到六條尾巴，落檔為 K-63～K-68**：K-63（「第一輪碰不到 BE」只斷言 tuning 值、沒玩任何一輪，還把可調數值釘死）與 K-64（視野門檻驗在合成 beat 上，真資料那三處寫錯鍵名照樣綠）**排 P2-E 動工前修**——P2-E 的八個指標之一就是「觸及 cap：從未」，真資料那一半沒釘住的話，P2-E 紅了會分不清是模擬過期還是引擎錯。其餘四條：K-65（`EffectApply` 在 BE 後仍套 `flag`，寫進新的一輪）排內容期之前、K-66（cap 數的來源與契約不同，目前恆等）下次動 `game_state.gd` 順手、K-67／K-68（BE 顯示測試沒驗到訊號接線；`tests/ui_sim/` 沒有 P2-D 契約）排 P2-E 收尾一批補。
- **下一步進 P2-E headless 重演三種玩家**。P2 順序：P2-A（✅）→ P2-B（✅）→ P2-C（✅）→ P2-D（✅）→ P2-E 重演三種玩家。**動工前先修 K-63／K-64。**
