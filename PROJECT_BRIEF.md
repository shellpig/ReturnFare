...
> **當前進度**：第一輪資料層完成（三章 beats 鋪滿、headless 驗證全綠）；四份關鍵文件建立完成；**Phase 1（最小可玩迴圈）P1-A～P1-F 已全部實作，P1-G 面板互動模型也已實作並 headless 全綠，45 天可貪心自動走查到底並正確回到第 1 天**；P1-F 收尾的待修條目（K-27～K-29、K-31、K-36～K-37、K-40）已全數修完。**P1 仍待 `測試指南.md > Phase 1` 的 50 條手動操作驗收——人還沒真的玩過一次。**（該節共 63 條，13 條標 headless 且已全綠。）**但那 50 條現在跑不動**——UI 模擬 runner 與「跳到第 N 天」的入口都還沒寫，深天數的條目要從第 1 天走 130 次以上互動過去。**下一步是先把工具做出來**（`開發設計方針.md > UI 模擬驗證`），不是硬跑。**P2 規格已寫完，可動工。**
...
- 2026-08-14：**P1-G 實作完成，headless 全綠，待手動驗收**。起因是手動驗收第一天就撞到的兩件事：光是好奇點進一個地點，`on_enter` 就結算了（看一眼要付代價）；而放卡之前完全看不到那一格收什麼。**不可逆本身是對的——那是本作的核心張力——錯的是把不可逆放在「進門」而不是「出手」。** P1-G 只搬這條界線，不給後悔藥：規則層拆成 `build_panel()`／`play_beat()`／`preview_slot()` 並刪除 `open_panel()`；進地點改成 beat 依序演出、演完才開放卡槽互動，槽常駐標型別並支援右鍵／觸控按鈕預覽，沒有卡放得下去時高亮推進鈕但不自動推進。白天夜間同一套。`locations.json.desc` 缺欄位時只顯示地點名，48 個描述文字留給內容角色。新增 P1-G headless 回歸與 lint 9。
- 2026-08-15：**建立 UI 模擬驗證的工具契約**（`開發設計方針.md > UI 模擬驗證`），並把 `測試指南.md > Phase 1` 的 50 條手動驗收逐條標上分工（🤖 runner／👤 人／🤖👤 兩者）與所需情境（新局／跳 Dn）。起因是那些驗收全住在 headless 看不見的那一層，但純手動的成本會壓垮驗收本身——走到第 32 天要 130 次以上互動，而且沒有存檔可跳，修完一個 bug 重驗又是一次全程。硬前提：**測試開始後不得直呼規則層**，一律走真實輸入事件（`Input.parse_input_event()`），禁用 `pressed.emit()`——用座標點才抓得到被遮住、越界、事件被攔截。經兩輪外部審核修掉十二條缺陷（`--state` 失敗不得靜默開新局、負向放置沒有 UI 路徑、右鍵預覽找不到目標、矩形診斷會誤報、資料變體缺傳入介面等）。**程式一行都還沒寫。**
- **下一步＝寫 runner，不是硬跑驗收。** 順序：`main.gd` 的 `--data-root` / `--state` 注入 → `qa_runner.gd`（`extends SceneTree`，自己起主場景）＋ `run_ui_sim.ps1` launcher → `qa_id` 掛上三個 UI 檔 → `qa_step.gd` 九步固定流程 → **跑通一個最簡單的案例**（P1-G 第 4 條槽型別標示，不跳天也不碰夜間）→ 拿實跑結果回填產物格式（dump 欄位、`report.json` schema、幾何診斷判準）→ `make_states.gd` 補決策與 checkpoint → P1-G 全部 14 條 → 其餘 33 條 → 完整走查。
