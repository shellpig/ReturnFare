...
headless 實測（2026-08-26，`verify_data.gd`）：**64 張卡／48 個地點（白天 20＋夜間 28）／18 個 NPC／261 個 beat**；引用 0 錯誤；地點三分類 10／10／16；lint 1～16 全 0 錯誤（lint 3 為 4 筆已豁免警告）；第 1–45 天行動格全覆蓋（第 1 天上午下午、第 32 天下午為刻意留空，名單抽至 `scripts/core/data_facts.gd` 共用）。
...
| P2-D 視野門檻與發瘋 BE | ✅ | 達 `madness_cap` 立即 BE、批次發卡只結束一次、共用既有 `end_run()` 收尾、BE 走獨立顯示分支（`test_p2d.gd` 全綠）。K-63／K-64、K-66／K-67 於 P2 收尾；K-68 於 P3-D 結案；K-65 於 P4-B 以 `run_generation` 防止 BE 後效果與文字污染新輪並結案。 |
...
| P4-A 資料與 SCHEMA 真值化 | ✅ | `delegation`／`encounter` schema、翻譯欄位、lint 15～16、D8／D17～19／D45 正式案例與委託臨時 gate 均已實作；`test_p4a` 含 graph traversal 防呆與 D8 cap 回歸，23 套 headless exit 0 |
| P4-B 委託規則 | ✅ | `delegate()` 原子入口、一天一人一次、immediate／隔日上午依序回報、序列化與換日重置、11 碼拒絕矩陣；接點失效保留且顯式報錯，K-65 同筆／跨筆效果與文字污染均封住。`test_p4b` 9 大測試段、全套 24 套 headless exit 0。 |
| P4-C 委託 UI 與首個案例 | ✅ | 委託候選／預覽／確認、第一張人物卡教學、D17～19 處方案例均已實作；K-124／K-125 結案，25 套 headless 全綠；完整 UI sim 104／104 variants、81／81 contracts、0 failed，11 條負向反證如期失敗。verifier targeted P4-C headless 與必要 UI 7／7 全綠；七條驗收依正式內容可達範圍與決策 (a) 完成證據收斂 |
| P4-D 遭遇規則 | ✅ | 遭遇狀態機、佔格、回應／錯答／丟棄／逃離、15 碼封閉拒絕矩陣與固定檢查順序、無合法解直接失敗；不消耗行動、不開縱慾備用區；K-126～K-147、K-149～K-150 全數結案，26 套 headless exit 0 |
...
- 2026-08-27：**P4-D 遭遇規則實作與複驗完成，K-126～K-147、K-149～K-150 全數結案，P4-D 文件關門並轉 ✅。** 遭遇狀態機、15 碼拒絕矩陣、三種出口、`encounter_view()` allowlist、DAG cycle lint 等均完成實作與變異測試。`test_p2_sim.gd` 補齊遭遇處理並通過 29 條指標斷言（K-144）；`run_all_headless.ps1` 採用檔案級重導向與 stderr 精確關鍵字比對守門（K-149）及動態專案路徑（K-150），完成雙向驗證（注入錯誤 exit 1、無注入全套 26 測 exit 0 且不誤傷故意 `push_error`）。**下一步依序實作 P4-E 遭遇 UI 與 D8／D45。**
- 2026-08-27：**P4-C 第三輪缺口經 verifier 複驗完成，文件重新關門並維持 ✅。** `247c716` 將阿婕信任破壞改走真實 `indulge(x_lust_ajie)`、修復走 D16 真實放卡、重取走 D17 `on_enter`；阿珠改走 D9 真實揭露槽，`task_title` 回填 view-model 契約，並新增縱慾後候選當場消失 UI 整合。verifier 自跑 `test_p4c.gd` 12 段 exit 0，必要 UI `p4c_01`～`p4c_07` 7／7、failed 0；完整 UI 證據為 104／104 variants、81／81 contracts、0 failed。測試指南 423／424 依使用者決策 (a) 以 headless／view-model 認列正式內容不可達的通用 UI 狀態，428／429 明記代表性 UI＋真實路徑 headless。**下一步 P4-D。**
- 2026-08-27：**K-125 經 verifier 複驗結案，P4-C 文件關門完成並轉 ✅。** `7a439f6` 讓 `drain_beats()` 優先以真實輸入關閉明示允許的委託教學 modal，dialog 與 beat 分開計數，並讓 D17 案例斷言教學 observation 與 `delegation_tutorial_seen`。verifier 先重跑原 4 個失敗變體全綠，再跑完整 UI sim：run `20260827-081934-525-p21024-40718853` 為 102／102 variants、79／79 contracts、0 failed，11 條負向反證如期失敗；合併 K-124 的 25 套 headless 證據後，P4-C 七條驗收全數打勾。**下一步依序實作 P4-D 遭遇規則，不先做 P4-E。**
- 2026-08-27：**P4-C 已實作，K-124 經 verifier 複驗結案；當時新發現 K-125 阻塞完整 UI 關門。** `4b0ba1c` 以 fixture-only madness 正規化修復 8 組精確張數情境，補張數／clock／day／phase 語意守衛，零人物卡改按 type 計數，並同批結 K-112（baseline 只在明示 `--p3a-baseline` 時產生）。verifier 自跑 fixture 生成 exit 0、baseline mtime 不變、25 套 headless 全綠；完整 UI sim 跑到 102 variants／79 contracts，98 variants 通過，4 個失敗同源於 D17 首張人物卡彈出的教學 modal 遮住底層 `beat_advance`，`drain_beats()` 卻只看 `is_visible_in_tree()` 而誤報無限迴圈。**此處原定的 K-125 關門條件已由上一筆 102／102、79／79、0 failed 證據完成。**
- 2026-08-26：**P4-B 委託規則實作完成並通過 verifier 複驗。** `delegate()`、`delegation_status()`、`delegates_used_today`、`pending_delegation_reports` 與 `run_generation` 接線完成；immediate／next_morning、一天一人一次、不吃主角行動、不增 `npc_action_counts`、choice 互斥、序列化、隔日上午順序及 11 碼拒絕矩陣均符合契約。兩輪 review 抓到延遲接點靜默丟棄、回報／強制縱慾順序守衛不足、evening 文件矛盾，以及 K-65 同筆／跨筆效果與文字污染；`52723df`、`ba6f9b8` 全數修復並補可辨識斷言。verifier 自跑 `test_p4b.gd` exit 0、正式資料 64／48／18／261 且 lint 1～16 全 0、全套 24 套 headless exit 0。**下一步依序實作 P4-C 委託 UI 與首個案例。**
- 2026-08-26：**P4-A 資料與 SCHEMA 真值化實作完成並通過 verifier 複驗。** 正式資料為 64 張卡／48 個地點／18 個 NPC／261 個 beat；引用與 lint 1～16 全綠。`test_p4a.gd` 覆蓋 lint 9、15、16 正負向、D8／D45 契約、D17 人物卡取得與委託映射、委託臨時 gate、`choice_requires_card` 成本，以及 D8 首次收費撞發狂上限後不污染新輪。最後一輪補成「壞第一 round＋合法第二 round」與 malformed fallback，確實咬住 graph traversal 防呆。verifier targeted 與全套 23 套 headless 均 exit 0。**下一步依序實作 P4-B 委託規則。**
