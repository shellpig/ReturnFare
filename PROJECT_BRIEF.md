...
- **K-48 結案的連帶影響**：**K-61（`tests/ui_sim/` 沒有 P2-C 契約）解除封鎖**，可以排了。另新增 **K-62**——`run_ui_sim.ps1` 用 `Set-Location` 決定測哪個專案，但 Godot 是用 `ProcessStartInfo` 起的、`--path .` 吃行程的工作目錄，所以從別的目錄呼叫會**安靜地去測別的專案**。診斷 K-48 時真的踩到了。
