...
- 2026-08-16：**P2-B 縱慾出口與主動縱慾實作與驗收完成**——新增 `data/beats/indulgence_exits.json`（定義山泉閣、老街、靜和園三個常駐 beat 與 7 個出口槽）、`DataFacts.beat_matches_time()` 收攏時段與日期區間比對、`Indulgence.level_for()` 實作強度級計算、`GameState.indulge()` 實作完整 8 步原子管線（含泡湯吃 2 格切換至 evening 特例與強度等級追加效果）、`try_place()` 轉導發狂卡至 `indulge()`、`PanelBuilder` 正確處理泡湯限制、`DataLoader` 實作 Lint 4（縱慾完整性保底）與 Lint 10（縱慾出口資料完整性）。壞資料 fixture 與專屬驗收測試 `test_p2b.gd`（10 組驗證）全綠，全部 12 套 headless 測試無迴歸。
- **下一步進 P2-C 強制縱慾與失控時段**。P2 順序：P2-A（✅）→ P2-B（✅）→ P2-C 強制縱慾與失控時段 → P2-D 視野縮減與 BE → P2-E 重演三種玩家。
