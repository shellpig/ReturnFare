...
> **當前進度**：第一輪資料層完成（三章 beats 鋪滿、headless 驗證全綠）；四份關鍵文件建立完成；**Phase 1（最小可玩迴圈）P1-A～P1-H 已全部實作並驗證全綠**；**Phase 2-A 發狂卡的產生與倒數已實作並驗收（`test_p2a.gd` 9 組全綠，全套 12 套 headless exit 0）**；**UI 模擬驗證工具鏈（`tests/ui_sim/`）53 條 UI 契約、66 個案例變體**。⚠️ **UI 模擬目前不可重現**——2026-08-16 乾淨環境連跑三輪，第 1、3 輪 53／53 全綠，第 2 輪 52／53（`驗證後已知問題.md > K-48`，已重開）。**下一步進 P2-B 縱慾出口與主動縱慾，但 K-48 要先收。**
...
| P2-B 縱慾出口與主動縱慾 | ✅ | 六個出口進 `data/beats/indulgence_exits.json`、`indulge()` 原子入口、泡湯特例（標記已用不跳時段、讀取 `soak_cards_cleared`）、`when.day_from` 常駐 beat、出口槽可重複使用（K-54）、Lint 4 & 10 檢查（`test_p2b.gd` 全綠、13 套 headless 全綠、UI 模擬 54 契約 67 變體全綠）。**UI 那一半沒有自動化**：`測試指南.md > P2-B` 的 11 條操作項驗在 view model 與規則層，`tests/ui_sim/` 沒有 P2-B 契約 |
...
- 2026-08-16：**P2-B 驗收走了兩輪，抓到四個缺口，全部修完。** 第一輪（`2cf88cb`）三條都出在「泡湯直接把 `phase` 改成 evening 並發 `phase_changed`」這一個根因上：① 上午泡湯之後推進鈕是灰的而且沒有人會解開，**這一輪玩不下去**（`_route_view()` 只設 visible，`disabled` 只有返回鍵會解，而返回鍵已經跟著面板藏了）② 泡湯的效果文字被寫進已隱藏的面板，玩家看到的是當晚殘響 ③ `soak_cards_cleared` 是死值，程式一張都沒讀。修法（`f7c95d9`）是照方針原本的契約走——`consume_action(n)` 改記 `_actions_spent_ahead`，不動 `phase`、不發訊號，放置管線恢復原子性；`advance_phase()` 進下一個行動時段時自動扣掉；`_route_view()` 另外補 `disabled = false`。第二輪抓到 **K-54：出口槽一輪只能用一次**——`indulge()` 寫 `slots_placed`，而出口 beat 的 `when` 是 `day_from: 1` → `day_to: 45`，那個鍵到 `end_run()` 才會清。實測一輪主動縱慾上限**只有 4 次**，而一輪供給 14 張發狂卡；這同時打到企劃書第七節「任何時刻都必須至少有一個出口點得下去」與規格書 P2-E「玩家 C 主動縱慾 14 次」。修法是 `indulge()` 不寫 `slots_placed`（節流閥是行動格，不是槽的一次性），修完上限回到 90。**`測試指南.md > P2-B` 的 15 條已打勾**，但那 11 條操作項是驗在 view model 與規則層——`tests/ui_sim/` 至今沒有 P2-B 契約，這是已知的覆蓋缺口。
- **下一步進 P2-C 強制縱慾與失控時段**。P2 順序：P2-A（✅）→ P2-B（✅）→ P2-C 強制縱慾與失控時段 → P2-D 視野縮減與 BE → P2-E 重演三種玩家。**P2-C 動工前先看 K-54**：`pick_exit()` 的規格只求值 `condition` 與 `requires`，不看 `slots_placed`，現在規則層與它一致了，不要又把一次性加回去。
