...
> **當前進度**：第一輪資料層完成（三章 beats 鋪滿、headless 驗證全綠）；四份關鍵文件建立完成；**Phase 1（最小可玩迴圈）P1-A～P1-F 已全部實作，P1-G 面板互動模型也已實作並 headless 全綠，45 天可貪心自動走查到底並正確回到第 1 天**；P1-F 收尾的待修條目（K-27～K-29、K-31、K-36～K-37、K-40）已全數修完。**P1 仍待 `測試指南.md > Phase 1` 的 49 條手動操作驗收——人還沒真的玩過一次。**（該節共 62 條，13 條標 headless 且已全綠。）**但那 49 條現在跑不動**——UI 模擬 runner 與「跳到第 N 天」的入口都還沒寫，深天數的條目要從第 1 天走 130 次以上互動過去。**下一步是先把工具做出來**（`開發設計方針.md > UI 模擬驗證`），不是硬跑。**P2 規格已寫完，可動工。**
...
- 2026-08-15：**建立 UI 模擬驗證的工具契約**（`開發設計方針.md > UI 模擬驗證`），並把 `測試指南.md > Phase 1` 的 49 條手動驗收逐條標上分工（🤖 runner／👤 人／🤖👤 兩者）與所需情境（新局／跳 Dn）。起因是那些驗收全住在 headless 看不見的那一層，但純手動的成本會壓垮驗收本身——走到第 32 天要 130 次以上互動，而且沒有存檔可跳，修完一個 bug 重驗又是一次全程。硬前提：**測試開始後不得直呼規則層**，一律走真實輸入事件（`Input.parse_input_event()`），禁用 `pressed.emit()`——用座標點才抓得到被遮住、越界、事件被攔截。經兩輪外部審核修掉十二條缺陷（`--state` 失敗不得靜默開新局、負向放置沒有 UI 路徑、右鍵預覽找不到目標、矩形診斷會誤報、資料變體缺傳入介面等）。**程式一行都還沒寫。**
- **下一步＝寫 runner，不是硬跑驗收。** 順序：`main.gd` 的 `--data-root` / `--state` 注入 → `make_states.gd` 產生狀態 → `qa_id` 掛上三個 UI 檔 → `qa_step.gd` 九步固定流程 → 拿 P1-G 第 3 條（第 1 天公車站）當第一個案例證明管線通 → 再擴到 P1-G 全部 14 條 → 最後才是其餘 35 條與完整走查。
