...
- **P2-C 還沒收的三條尾巴**（2026-08-20 落檔為 `驗證後已知問題.md > K-59／K-60／K-61`）：**K-59** ③的防吃債修對了但**沒有測試守著**（要壞資料 fixture 才走得到，改回舊寫法 14 套照樣全綠）；**K-60** ②的判定綁在「出口 `on_place` 有文字」，哪天有出口只寫效果不寫文字，走查會假紅；**K-61** `tests/ui_sim/` **沒有任何 P2-C 契約**，操作層只驗到規則層。K-59 與 K-60 排在 P2-D 動工前同一批修；**K-61 要等 K-48 收掉才做**——UI 模擬跑不穩的時候補契約，綠紅都不算數。
