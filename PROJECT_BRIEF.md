...
- 2026-08-20：**P3 三份文件的規格審核完成，九條缺口落檔為 K-73～K-81。** 規格與資料對得上（28 row／12 對位／10 卡／16 夜間限定／6 門檻／14 收費 row 全部實測相符），**問題全部在沒有人把數值後果算過**：**K-73**——全作發狂卡只有 marker cost 與 `n3_map_opens` 三個槽兩個來源，後者 P3-C 要刪、前者 P3-B 改成終身首次，**第二輪供給因此是 0**，倒數／強制縱慾／視野門檻／發瘋 BE 在第二輪全部不會發生，而 P3-F 把它寫成驗收綠燈；**K-74**——`n_plaza` 是 14 個收費 row 裡唯一掛 night-layer fixed 的，D15 讓它永久免費，**第一輪供給 14 → 13**，而 `實作規格書.md > P3-F` 寫「仍逐項對上 P2 模擬」，與方針／測試指南的「不一致就更新基準」互相打架。**K-73／K-74 要在 P3-A 開工前處理**，其餘七條各自綁在對應子階段開工時；清單見 `驗證後已知問題.md > 一、待修清單`。
