...
## P2-A headless 驗收測試：發狂卡的產生、獨立倒數、tick_madness、open_night_marker、序列化往返。
## 跑法：
...
func _initialize() -> void:
	...
	failed += _test_daily_countdown_and_zeroing(gs, data_node)
...
func _test_single_card_generation_night_marker(gs: Node, _data_node: Node) -> int:
	...
	if not opened.has("n_ahong_1"):
		...
		failed += _ok("night_markers_opened 記錄 n_ahong_1")
...
# ── 2. 每天 morning 減 1，第 13 天 morning 歸零 ─────────────────────────────

func _test_daily_countdown_and_zeroing(gs: Node, _data_node: Node) -> int:
	print("--- 2. daily countdown and zeroing on Day 13 morning ---")
	var failed := 0
	_reset_gs(gs)
	...
	# 逐日推進至第 12 天 night
	# 每天 4 時段 (morning -> afternoon -> evening -> night)
	for d in range(7, 13):
		# 目前在 d morning，推進 3 次到 d night
...
func _test_reentry_idempotent(gs: Node, _data_node: Node) -> int:
	print("--- 4. reentry to opened night marker is idempotent ---")
	var failed := 0
	_reset_gs(gs)
	...
	if lines2.is_empty() and hand_count_1 == hand_count_2 and clock_count_1 == clock_count_2:
		failed += _ok("已開過的地點再進不再發卡，手牌張數不變")
	else:
		failed += _fail("重複進入收費地點重複發卡 (hand1=%d, hand2=%d)" % [hand_count_1, hand_count_2])
...
func _test_free_location_no_madness(gs: Node, _data_node: Node) -> int:
	print("--- 5. free location (madness_cost == 0) does not grant madness ---")
	var failed := 0
	_reset_gs(gs)

	gs.set("day", 10)
	gs.set("phase", "night")
	gs.call("open_night_marker", "n_landmark")

	var hand: Array = gs.get("hand")
	var clock: Dictionary = gs.get("madness_clock")
	var opened: Dictionary = gs.get("night_markers_opened")

	if hand.size() == 1 and clock.is_empty() and opened.has("n_landmark"):
		failed += _ok("進入免費地點 n_landmark 記入 opened 但不發放發狂卡")
	else:
		failed += _fail("免費地點異常發卡或未記入 opened (hand=%s, clock=%s, opened=%s)" % [str(hand), str(clock), str(opened)])

	return failed


# ── 6. fixed 定日夜 beat 不算開標記不發卡 ─────────────────────────────────────

func _test_fixed_night_beat_no_marker_no_madness(gs: Node, _data_node: Node) -> int:
	print("--- 6. fixed night beat does not count as marker ---")
	var failed := 0
	_reset_gs(gs)
...
func _test_serialize_roundtrip_p2a(gs: Node) -> int:
	print("--- 7. serialization roundtrip with P2-A fields ---")
	var failed := 0
	_reset_gs(gs)
	...
	gs.set("night_markers_opened", { "n_ahong_1": true, "n_source": true, "n_landmark": true })
	...
	if restored_opened.has("n_ahong_1") and restored_opened.has("n_source") and restored_opened.has("n_landmark"):
		failed += _ok("序列化還原 night_markers_opened 集合正確")
	else:
...
# ── 8. end_run 完整重置 P2-A 欄位 ───────────────────────────────────────────

func _test_end_run_resets_p2a(gs: Node) -> int:
	print("--- 8. end_run resets P2-A state fields ---")
	var failed := 0
	...
	if clock.is_empty() and counter == 0 and opened.is_empty() and ind_cnt == 0 and pending.is_empty():
		failed += _ok("end_run 成功清空 madness_clock, _madness_counter, night_markers_opened, indulgence_count, forced_pending")
	else:
		failed += _fail("end_run 未清空 P2 欄位 (clock=%s, counter=%d, opened=%s, ind_cnt=%d, pending=%s)" % [
			str(clock), counter, str(opened), ind_cnt, str(pending)
		])
